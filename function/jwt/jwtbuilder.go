...
func init() {
	_ = function.Register(&fnGenerateJWT{})
}

type fnGenerateJWT struct {
}

func (s *fnGenerateJWT) Name() string {
	return "generateJWT"
}

func (fnGenerateJWT) Sig() (paramTypes []data.Type, isVariadic bool) {
	return []data.Type{data.TypeString, data.TypeString, data.TypeString, data.TypeString, data.TypeInt}, false
}

func (fnGenerateJWT) Eval(params ...interface{}) (interface{}, error) {
	...
	if !strings.Contains(privateKey, "\n") {
		beginString := strings.Split(privateKey, "-----BEGIN PRIVATE KEY----- ")
